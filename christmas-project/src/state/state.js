...
export function addToRecentlyViewed(employeeId) {
  const filtered = state.recentlyViewed.filter((id) => id !== employeeId);
  filtered.unshift(employeeId);
  state = {
    ...state,
    recentlyViewed: filtered.slice(0, 5),
  };
}
 ...
 */
export function updateEmployeeInState() {
